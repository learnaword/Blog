var globalCount = 0;
  ...
  $(window).scroll(function() {
    ...
    if ($(document).scrollTop() + 50 >= $(document).height() - $(window).height() && width > 700) {
      ...
      setTimeout(function() {
        initBlogListByPage(pageNext, type_id, type_name);
      }, 500);
    }
  });
  ...
  var initBlogListByPage = function(pageNum, typeId, typeName) {
    //查询出文章
    //获取关键字，表示查询所有符合的记录
    var params = "";
    var prarm = "";
  ...
  var searchType = function(typeId, typename) {
    var index = '';
    layer.ready(function() {
      index = layer.load(2, {
        shade : [ 0.1, '#eee' ] //0.1透明度的白色背景
