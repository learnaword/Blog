		...
		success : function(data) {
			if (data.status == 200) {
				...
				for (var i = 0; i < data.length; i++) {
					//计算现在时间与日志时间的差值
					dateTocreateTime = now - data[i].createTime;
					time = parseInt(dateTocreateTime / minutes);
					if (time < 60) {
						if (time == 0) {
