...
$(document).ready(function() {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (data.status == 200) {
				...
				//加载编辑页面预览的  时间和浏览量
				$(".newsview").find(".au02").html(Format(data.blog.createTime, "yyyy-MM-dd hh:mm:ss"));
				$(".au03").find('b').html(data.blog.clicknum);
                                $("#clicknum").val(data.blog.clicknum);
				...
				var typeName = '';
				typeName += '<option style="color:#000;font-weight:bold;" value="' + data.blog.type.id + '" selected="selected">' + data.blog.type.typename + '</option>';
				$(".form-horizontal").find('select[name=typeName]').append(typeName);
                var softName = '';
                softName += '<option style="color:#000;font-weight:bold;" value="' + data.blog.soft.id + '" selected="selected">' + data.blog.soft.title + '</option>';
                $(".form-horizontal").find('select[name=soft]').append(softName);
				var agreenumName = '';
				agreenumName += '<option style="color:#000;font-weight:bold;" value="' + data.blog.agreenum + '" selected="selected">' + agreenum[data.blog.agreenum]+ '</option>';
				$(".form-horizontal").find('select[name=agreenum]').append(agreenumName);

                                var commentnumName = '';
                                commentnumName += '<option style="color:#000;font-weight:bold;" value="' + data.blog.commentnum + '" selected="selected">' + commentnum[data.blog.commentnum]+ '</option>';
                                $(".form-horizontal").find('select[name=commentnum]').append(commentnumName);
			}
		},
		error : function() {
	...
	var label = new Array(
		...
		"label-success",
		"label-warning");
	for (var i = 0; i < agreenum.length; i++) {
		agreenumName += '<option value="' + i + '">'
			+ agreenum[i]
			+ '</option>';
	}
	// 初始化数据
	...
	$.ajax({
		...
		success : function(data) {
			...
			swal({
				...
				setTimeout(function() {
					window.location.href = "../blog/updateBlog.jsp?id="+(parseInt(url_param) -1);
				}, 1000);
			} else {
