...
var globalCount = 0;
  ...
  $(window).scroll(function() {
    ...
    if ($(document).scrollTop() + 50 >= $(document).height() - $(window).height() && width > 700) {
      ...
      setTimeout(function() {
        initSoftListByPage(pageNext, "none", null);
      }, 500);
    }
  });
  ...
  $(document).ready(function() {
    ...
    if(keyword.search('type')!=-1){
      var id = keyword.split('_')[1];
        initSoftListByPage(pageNext, id, id);
    }else{
        initSoftListByPage(pageNext, "none", null);
    }
  });
  
  var initSoftListByPage = function(pageNum, type_id, typename) {
    //查询出文章
    //获取关键字，表示查询所有符合的记录I
    var params = "";
    var prarm = "";
      ...
      success : function(data) {
        ...
        if (data.length > 0) {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            parm += data[i].id + ",";
            var id = data[i].id;
           /* blogList += '<li style="animation-delay:0.' + i + 's" class="animated fadeInDown"><h3 class="blogtitle"><a href="find/' + id + '.html"  >'
            + data[i].title
            ...
            + Format(data[i].createTime, "yyyy-MM-dd")
         ...
         */
            blogList += '<li style="animation-delay:0.' + i + 's" class="animated fadeInDown"><h3 class="blogtitle"><a href=softInfo?keyboard=type_'+data[i].id +'"  >'
            + data[i].title
            + '</a></h3><span class="blogpic imgscale"><a href=softInfo?keyboard=type_'+data[i].id +'" title=""><img src="' + data[i].images + '"  /></a></span><p class="blogtext">'
            + data[i].introduction
            + '</p><p class="bloginfo"><i class = "avatar"><img src="images/image_.jpg" border=0 width="30" height="30"></i><span>house</span><span><a href="javascript:void(0);">【'
            + data[i].type.typename
            + '】</a></span><span class="m_time">'
            + Format(data[i].createTime, "yyyy-MM-dd")
            + '</span><span class="f_r"></p><a href="softInfo?keyboard=type_'+data[i].id +'" class="viewmore">进入专区</a></span></li>'
  ...
  var initBlogByTop = function() {
    ...
    $.ajax({
      ...
      success : function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
          var id = data[i].id;
          topBlog +='<li><a href="find/' + id + '.html" title='+ data[i].title + '>'+data[i].title+'</a></li>';
        }
        // 初始化数据
  ...
  var searchType = function(type_id, typename) {
    ...
    initSoftListByPage(1, type_id, typename);
    setTimeout(function() {
      layer.close(index);
    }, 200);
      ...
      success : function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (i == 1) {
            ...
            likeBlog_2 += '<li style="animation-delay:' + time + 's" class="animated fadeIn"> <i><img src="' + data[i].images + '"></i><p><a href="find/' + id + '.html">' + data[i].title + '</a></p><span>[' + Format(data[i].createTime, "yyyy-MM-dd") + ']</span> </li>';
          }
        }
        // 初始化数据
    ...
    $.ajax({
      ...
      success : function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          var id = data[i].id;
          time = i * 0.05;
          clickBlog += '<li style="animation-delay:0.' + i + 's" class="animated fadeIn"><i></i><a href="find/' + id + '.html">'
            + data[i].title
            + '</a></li>'
        }
        // 初始化数据
