...
$(document).ready(function() {
	...
	layui.use(['table','laytpl'], function () {
		...
		$('#submit_btn').on('click', function () {
			...
			request.put("/admin/technology/blog/update", formData).then(function(data){
				if(data.data.code == "0"){
					Swal.fire({
						type: 'success', // 弹框类型
						title: '更新文章', //标题
...
function renderSoft() {
	let url = "/admin/technology/soft/list";
...
function renderAd() {
	let url = "/admin/technology/recommend/list";
	request.get(url).then(function (data) {
		if (data != null) {
			//数据渲染到html
			var responseData = data.data.data;
...
function initBlog() {
	const id = getURLParameter('id');
	let url = "/admin/technology/blog/get?id=" + id;
