...
$(document).ready(function(){
    layui.use(['table', 'dropdown'], function(){
        ...
        table.render({
            elem: '#test'
            ,url:'/admin/technology/blog/table' // 此处为静态模拟数据，实际使用时需换成真实接口
            ,toolbar: '#toolbarDemo'
            ,headers: {
        ...
        table.on('tool(test)', function(obj){ // 双击 toolDouble
            ...
            // console.log(obj)
            if(obj.event === 'edit'){
                window.location.href = "/page/admin/technology/blog/update.jsp?id="+data.id;
...
export function updateBlogsStatus(selectedIds,status,msg){
    request.put("/admin/technology/blog/update-status", {ids:selectedIds,status: status}).then(function(data){
        Swal.fire({
            type: 'warning', // 弹框类型
            title: msg + '操作', //标题
            text: msg + "成功！", //显示内容
...
export function updateBlogsRecommend(selectedIds,isRecommend){
    request.put("/admin/technology/blog/update-recommends", {ids:selectedIds,isRecommend: isRecommend}).then(function(data){
        Swal.fire({
            type: 'warning', // 弹框类型
            title: '设置推荐', //标题
            text: "设置成功！", //显示内容
...
export function updateBlogsTop(selectedIds,isTop){
    request.put("/admin/technology/blog/update-tops", {ids:selectedIds,isTop: isTop}).then(function(data){
        Swal.fire({
            type: 'warning', // 弹框类型
            title: '设置置顶', //标题
            text: "设置成功！", //显示内容
