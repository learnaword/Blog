...
$(document).ready(function(){
    layui.use(['table', 'dropdown'], function(){
        ...
        table.render({
            elem: '#test'
            ,url:'/admin/recommend/table' // 此处为静态模拟数据，实际使用时需换成真实接口
            ,toolbar: '#toolbarDemo'
            ,headers: {
            ...
            ,cols: [[
                ...
                ,{field:'smallTitle', title:'小标题', width: 80}
                ,{field:'introduction', title:'介绍', width: 120}
                ,{field:'buttonInfo', title:'按钮信息', width: 80}
                ,{field:'buttonBottom', title:'按钮底部信息', width: 80}
                ,{field:'buttonLink', title:'按钮链接', width: 80}
                ,{
        ...
        table.on('tool(test)', function(obj){ // 双击 toolDouble
            ...
            // console.log(obj)
            if(obj.event === 'edit'){
                window.location.href = "/page/admin/recommend/update.jsp?id="+data.id;
...
export function updateStatus(selectedIds,status,msg){
    request.put("/admin/recommend/update-status", {ids:selectedIds,status: status}).then(function(data){
        Swal.fire({
            type: 'warning', // 弹框类型
            title: msg + '操作', //标题
            text: msg + "成功！", //显示内容
