...
$(document).ready(function() {
    ...
    $("#startBlog").val(start);
    $("#endBlog").val(end);
    initBlogCountByDate(start,end); //博客发表量
});
...
var initEchartsByVisit = function(days, counts,num) {
...
var initBlogCountByDate = function(startTime, endTime) {
    ...
    var end = Date.parse(new Date(endTime));
    var num = Math.abs(parseInt((end - start) / 1000 / 3600 / 24));
    request.get('/admin/echars/blog-data?start='+start+"&end="+end).then(function(res){
        initEchartsByBlog(res.data.data.days, res.data.data.counts,num);
    });
};
        ...
        success : function(data) {
            ...
            for (var j = 0; j < days.length; j++) {
                for (var i = 0; i < data.list.length; i++) {
                    var time = Format(data.list[i].createTimeTmp, "yyyy-MM-dd");
                    if (days[j] == time) {
