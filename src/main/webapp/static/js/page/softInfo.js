...
$(window).scroll(function() {
    ...
    if ($(document).scrollTop() + 50 >= $(document).height() - $(window).height() && width > 700) {
        ...
        setTimeout(function() {
            ...
            var soft_id = keyword.split('_')[1];
            initBlogListByPage(pageNext, soft_id);
        }, 500);
    }
});
...
$(document).ready(function() {
    ...
    if(keyword.search('type')!=-1){
        ...
        initBlogListByPage(pageNext, "none");
    }
});

var initBlogListByPage = function(pageNum, soft_id) {
    //查询出文章
    //获取关键字，表示查询所有符合的记录
    var params = "";
    var prarm = "";
            ...
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (data[i].keyword != "" && data[i].keyword != null) {
                      ...
                      blogList += '<li style="animation-delay:0.' + i + 's" class="relart animated fadeInDown" onclick="window.location.href=\'https://www.bangmangma.com/find/'
                        + data[i].id
                        + '.html\'">'
                        +'<h3 style="font-weight:bold;" class="blogtitle"><a href="find/' + id + '.html"  >'
                        + data[i].title
...
var searchType = function(soft_id) {
    var index = '';
    layer.ready(function() {
        index = layer.load(2, {
            shade : [ 0.1, '#eee' ] //0.1透明度的白色背景
