package com.mjl.blog.controller.admin.technology.blog;
...
import com.mjl.blog.service.admin.technology.blog.TechnologyBlogAdminService;
...
@RequestMapping("/admin/technology/blog")
@RestController
@Validated
public class TechnologyBlogAdminController {

    @Resource
    private TechnologyBlogAdminService blogAdminService;
    ...
    *获取前preNum天的文章
    ...
    public CommonResult<BlogRespVO> get(Long id){
        return success(TechnologyBlogAdminConvert.INSTANCE.convert(blogAdminService.getBlogById(id)));
    }

    @GetMapping("/table")
    @Operation(summary = "获取文章表单")
    public CommonResult<PageResult<BlogTableRespVO>> getBlogTable(BlogTableReqVO blogTableReqVO){
        return success(TechnologyBlogAdminConvert.INSTANCE.convert(blogAdminService.getBlogList(blogTableReqVO)));
    }
